...
我们看这道题的特殊性：`字典序最大的出栈顺序`, 考虑`贪心策略` ： 因为是字典序最大， 所以一直入栈到 n， 之后考虑 n-1， 如果 n-1 在 n 之后的话， 就一直继续入栈到 n-1； 如果 n-1 在 n 之前(n-1 已经在栈中了)， 那就比较栈顶的元素和 n-1 的大小。 n 之后元素同找 n 一样， 最后弹出栈中的所有元素即可。  
